 ...
 */
import { isIntentKind, isPerpIntentKind, postsZeroCollateral, } from "./contracts.js";
 ...
 */
export function staticGuards(policy, intent, opts) {
    if (policy.killSwitch) {
        throw new GuardError("KILL_SWITCH", "kill switch is engaged — all trading is halted");
    }
    if (!opts.dryRun && !policy.executionEnabled) {
        throw new GuardError("EXECUTION_DISABLED", "execution is disabled — the kernel is in dry-run; arm it before trading");
    }
    if (!isIntentKind(intent.kind)) {
        throw new GuardError("INVALID_INTENT", `unsupported intent kind: ${String(intent.kind)}`);
    }
    ...
    if (postsZeroCollateral(intent.kind)) {
        if (intent.input.amount < 0n) {
            throw new GuardError("INVALID_INTENT", "input amount must not be negative");
        }
    }
    ...
    if (isPerpIntentKind(intent.kind)) {
        if (!intent.perp) {
            throw new GuardError("INVALID_INTENT", `intent kind ${intent.kind} is missing its perp leg`);
        }
        if (intent.perp.minBaseAmount > intent.perp.expectedBaseAmount) {
            throw new GuardError("INVALID_INTENT", "perp minimum base size exceeds the expected size");
        }
        if (intent.perp.expectedBaseAmount <= 0n) {
            throw new GuardError("INVALID_INTENT", "perp expected base size must be positive");
        }
    }
    else if (intent.perp) {
        throw new GuardError("INVALID_INTENT", `intent kind ${intent.kind} must not carry a perp leg`);
    }
    if (!intent.unsignedTxBase64) {
        throw new GuardError("INVALID_INTENT", "intent is missing its built transaction");
    }
    // Untrusted (model/metadata-sourced) mints require an explicit human confirmation.
    if ((intent.inputProvenance === "untrusted" ||
        intent.outputProvenance === "untrusted") &&
        !opts.confirmedByUser) {
        throw new GuardError("MINT_NOT_PINNED", "a mint in this trade is not pinned by you — explicit confirmation required");
    }
    ...
    if (intent.priorityFeeLamports < 0 ||
        ...
        throw new GuardError("PRIORITY_FEE_INVALID", "priority fee must be a finite, non-negative lamport amount");
    }
    const feeLamports = BigInt(Math.floor(intent.priorityFeeLamports));
    if (feeLamports > policy.priorityFeeMaxLamports) {
        throw new GuardError("PRIORITY_FEE_EXCEEDED", `priority fee ${feeLamports} lamports exceeds the ${policy.priorityFeeMaxLamports} lamport ceiling`);
    }
    ...
    if (quoteBucketFor(intent.input.mint) && intent.input.amount > 0n) {
        const bpsCeiling = (intent.input.amount * BigInt(policy.priorityFeeMaxBps)) / 10000n;
        if (feeLamports > bpsCeiling) {
            throw new GuardError("PRIORITY_FEE_EXCEEDED", `priority fee ${feeLamports} lamports exceeds ${policy.priorityFeeMaxBps}bps of notional (${bpsCeiling})`);
        }
    }
    ...
    if (intent.quote.slippageBps > policy.maxSlippageBps) {
        throw new GuardError("SLIPPAGE_EXCEEDED", `slippage ${intent.quote.slippageBps}bps exceeds max ${policy.maxSlippageBps}bps`);
    }
    // The route's worst-case out must be consistent with the clamped slippage.
    const impliedBps = slippageBps(intent.quote.outAmount, intent.quote.minOutAmount);
    if (impliedBps > policy.maxSlippageBps + MIN_OUT_TOLERANCE_BPS) {
        throw new GuardError("MIN_OUT_MISMATCH", `route min-out implies ${impliedBps}bps slippage, above the clamp`);
    }
    ...
    for (const mint of [intent.input.mint, intent.output.mint]) {
        ...
        if (policy.mintAllowlist && !policy.mintAllowlist.includes(mint)) {
            throw new GuardError("MINT_DENIED", `mint ${mint} is not on the allowlist`);
        }
    }
}
