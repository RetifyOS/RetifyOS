...
export class Reconciler {
    ...
    async recover() {
        ...
        for (const trade of pending) {
            ...
            const conf = await confirmer.confirm(trade.signature, trade.last_valid_block_height);
            if (conf.status === "confirmed") {
                ...
                if (trade.reservation_id)
                    store.releaseReservation(trade.reservation_id);
                store.fail(trade.id, conf.status === "expired" ? "expired" : "errored", `RECONCILE_${conf.status.toUpperCase()}`, clock.now());
                failed += 1;
            }
        }
