...
const FEE_BUFFER_LAMPORTS = 3000000n;
...
export class TradeGatewayImpl {
    ...
    async #preview(intent, opts) {
        ...
        try {
            ...
            if (!sim.ok) {
                throw new GuardError("SIMULATION_FAILED", "preflight simulation failed", { err: String(sim.err ?? "unknown") });
            }
    ...
    async #live(intent, opts) {
        ...
        try {
            ...
            if (bucket) {
                ...
                if (!outcome.ok) {
                    throw new GuardError("CAP_EXCEEDED", `${bucket.toUpperCase()} ${outcome.reason} cap exceeded`, {
            ...
            const need = intent.input.amount + (bucket === "sol" ? FEE_BUFFER_LAMPORTS : 0n);
            if (pre.input < need) {
                throw new GuardError("INSUFFICIENT_BALANCE", "wallet balance cannot cover this trade plus fees", { have: pre.input.toString(), need: need.toString() });
            }
            ...
            store.persistSigned(tradeId, signed.wireBase64, signed.signature, clock.now());
            store.appendJournal({
                type: "trade.signed",
                tradeId,
                at: clock.now(),
            ...
            // 7. Broadcast.
            const broadcaster = this.#d.broadcasters[intent.landMode];
            if (!broadcaster) {
                throw new GuardError("BROADCAST_FAILED", `no broadcaster for land mode '${intent.landMode}'`);
            }
            let signature;
            try {
                ({ signature } = await broadcaster.broadcast(signed, intent.landHandle));
            }
            catch (e) {
                throw new GuardError("BROADCAST_FAILED", e instanceof Error ? e.message : String(e));
            }
            ...
            const conf = await this.#d.confirmer.confirm(signature, intent.lastValidBlockHeight);
            if (conf.status !== "confirmed") {
                const code = conf.status === "expired" ? "CONFIRM_TIMEOUT" : "BROADCAST_FAILED";
                if (reservationId)
                    store.releaseReservation(reservationId);
                store.fail(tradeId, conf.status === "expired" ? "expired" : "errored", code, clock.now());
                store.appendJournal({
                    type: "trade.failed",
                    tradeId,
                    at: clock.now(),
                ...
                return this.#errorResult(tradeId, new GuardError(code, `transaction ${conf.status} before confirmation`), false);
            }
            ...
            if (reservationId)
                store.consumeReservation(reservationId);
     ...
     */
    #assertSettleable(intent) {
        if (settleModeFor(intent.kind) !== "venue-position")
            return;
        if (!this.#d.positions) {
            throw new GuardError("SETTLE_UNVERIFIABLE", `no position reader is mounted for venue '${intent.perp?.venue ?? "unknown"}' — ` +
     ...
     */
    async #settle(intent, outputDelta, prePosition) {
        const perp = intent.perp;
        const ref = this.#positionRef(intent);
        if (!ref || !perp || prePosition === undefined) {
            const effectiveSlippageBps = slippageBps(intent.quote.outAmount, outputDelta > 0n ? outputDelta : 0n);
            return {
                effectiveSlippageBps,
                positionDelta: undefined,
                problem: outputDelta < intent.quote.minOutAmount
                    ? {
        ...
        try {
            ...
            return {
                ...
                problem: {
                    ...
                    message: `confirmed on-chain, but the ${perp.venue} position could not be read back: ${e instanceof Error ? e.message : String(e)}`,
                },
            };
        }
        const positionDelta = postPosition - prePosition;
        const filled = perp.side === "long" ? positionDelta : -positionDelta;
        const effectiveSlippageBps = slippageBps(perp.expectedBaseAmount, filled > 0n ? filled : 0n);
        return {
            effectiveSlippageBps,
            positionDelta,
            problem: filled < perp.minBaseAmount
                ? {
                    code: "SETTLE_SHORTFALL",
    ...
    async #assertMintsSupported(policy, intent) {
        ...
        if (!policy.allowToken2022 && (inInfo.isToken2022 || outInfo.isToken2022)) {
            throw new GuardError("TOKEN2022_UNSUPPORTED", "Token-2022 mint detected; refused (transfer fees / hooks are not accounted for)");
        }
    }
    #assertWithinCaps(bucket, amount, caps, used) {
        if (amount > caps.perTrade) {
            throw new GuardError("CAP_EXCEEDED", `${bucket.toUpperCase()} per-trade cap exceeded`);
        }
        if (used.hour + amount > caps.perHour) {
            throw new GuardError("CAP_EXCEEDED", `${bucket.toUpperCase()} per-hour cap exceeded`);
        }
        if (used.day + amount > caps.perDay) {
            throw new GuardError("CAP_EXCEEDED", `${bucket.toUpperCase()} per-day cap exceeded`);
        }
    }
    async #readPair(intent) {
    ...
    #errorResult(tradeId, err, simulated) {
        const code = isGuardError(err) ? err.code : "INTERNAL";
...
function result(tradeId, state, extra = {}) {
    return {
        tradeId,
        state,
