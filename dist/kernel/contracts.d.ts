 ...
 */
export declare const TRADE_STATES: readonly ["reserved", "sent", "confirmed", "expired", "errored", "rejected"];
 ...
 */
export declare const PERP_INTENT_KINDS: readonly ["perp_open", "perp_increase", "perp_reduce", "perp_close"];
export type PerpIntentKind = (typeof PERP_INTENT_KINDS)[number];
/** Concentrated-liquidity and bonding-curve kinds. */
export declare const POOL_INTENT_KINDS: readonly ["lp_open", "lp_add", "lp_remove", "lp_close", "lp_claim", "lp_rebalance", "curve_buy", "curve_sell"];
 ...
 */
export declare const INTENT_KINDS: readonly ["swap", "perp_open", "perp_increase", "perp_reduce", "perp_close", "lp_open", "lp_add", "lp_remove", "lp_close", "lp_claim", "lp_rebalance", "curve_buy", "curve_sell"];
export type IntentKind = (typeof INTENT_KINDS)[number];
export declare function isIntentKind(kind: string): kind is IntentKind;
export declare function isPerpIntentKind(kind: string): kind is PerpIntentKind;
/**
 ...
 */
export declare function postsZeroCollateral(kind: IntentKind): boolean;
/**
...
export declare function settleModeFor(kind: IntentKind): SettleMode;
...
export interface Broadcaster {
    ...
    broadcast(signed: SignedTx, landHandle: string | undefined): Promise<{
        signature: string;
    }>;
}
...
export interface Confirmer {
    confirm(signature: string, lastValidBlockHeight: number, signal?: AbortSignal): Promise<ConfirmOutcome>;
}
 ...
 */
export declare const TOOL_CAPABILITIES: readonly ["read", "sign", "spend", "network", "read_state", "write_state"];
export type ToolCapability = (typeof TOOL_CAPABILITIES)[number];
export declare const TOOL_CATEGORIES: readonly ["data", "swap", "perps", "lending", "lp", "launchpad", "staking", "nft", "wallet", "notify"];
...
export interface IntentToolDefinition<Cfg = unknown> {
    ...
    execute(ctx: ToolContext, cfg: Cfg, opts: ToolExecOptions): Promise<ToolOutcome>;
}
export declare function hasToolCapability(tool: Pick<IntentToolDefinition, "capabilities">, cap: ToolCapability): boolean;
/** A tool moves value iff it declares `sign` or `spend`. */
export declare function movesValue(caps: readonly ToolCapability[]): boolean;
