...
import { ControlAuth, readCookie, isSameOrigin, SESSION_COOKIE, } from "./auth.js";
 ...
 */
export function registerControlPlane(app, options) {
    const auth = options.auth instanceof ControlAuth
        ? options.auth
    ...
    if (!auth.configured) {
        ...
        console.warn("[RetifyOS] control plane mounted WITHOUT a credential: set API_BEARER_TOKEN or API_BEARER_TOKEN_SHA256. Every /api route — including approvals — will refuse with 401 AUTH_NOT_CONFIGURED.");
    }
    ...
    // ── identity ──────────────────────────────────────────────────────────────
    const secure = (req) => req.protocol === "https";
    ...
    const guard = (req, reply, scope) => {
        if (!auth.configured) {
            void fail(reply, 401, "AUTH_NOT_CONFIGURED", "this daemon has no API credential configured, so the console cannot be authenticated; set API_BEARER_TOKEN or API_BEARER_TOKEN_SHA256 and restart");
            return false;
        }
    ...
    const writable = (req, reply) => {
        const type = String(req.headers["content-type"] ?? "");
        if (!type.toLowerCase().startsWith("application/json")) {
            void fail(reply, 415, "UNSUPPORTED_MEDIA_TYPE", "requests that change state must be application/json");
            return false;
        }
        if (!isSameOrigin({
    ...
    app.post(`${prefix}/session`, async (req, reply) => {
        if (!writable(req, reply))
            return;
        if (!auth.configured)
            return fail(reply, 401, "AUTH_NOT_CONFIGURED", "this daemon has no API credential configured; set API_BEARER_TOKEN or API_BEARER_TOKEN_SHA256 and restart");
        const ip = req.ip ?? "unknown";
        const now = Date.now();
        const seen = attempts.get(ip);
        const window = !seen || now - seen.start >= SESSION_ATTEMPT_WINDOW_MS
            ? { n: 1, start: now }
    ...
    app.get(`${prefix}/session`, async (req, reply) => reply.send({
    ...
    app.get(`${prefix}/snapshot`, async (req, reply) => {
        if (!guard(req, reply, CONTROL_SCOPES.read))
            return;
        return reply
    ...
    app.get(`${prefix}/stream`, async (req, reply) => {
        ...
        const push = async () => {
            ...
            try {
                ...
                if (!stopped)
                    raw.write(": snapshot unavailable\n\n");
            }
        };
    ...
    // ── writes ────────────────────────────────────────────────────────────────
    app.post(`${prefix}/approvals/:id/decide`, async (req, reply) => {
        ...
        if (!guard(req, reply, CONTROL_SCOPES.approve))
            return;
        if (!writable(req, reply))
            return;
        const decision = body(req).decision;
        if (decision !== "approve" && decision !== "reject")
            return fail(reply, 400, "VALIDATION_ERROR", "decision must be 'approve' or 'reject'");
        return fail(reply, 503, "APPROVALS_UNAVAILABLE", "this build has no pending-approval queue: the kernel takes `confirmedByUser` per execute call and the composition leaves it unwired, so there is no intent to bind a decision to");
    });
    const policyRoute = (path, field, apply) => app.post(`${prefix}${path}`, async (req, reply) => {
        if (!guard(req, reply, CONTROL_SCOPES.policy))
            return;
        if (!writable(req, reply))
            return;
        const value = body(req)[field];
        if (typeof value !== "boolean")
            return fail(reply, 400, "VALIDATION_ERROR", `${field} must be a boolean`);
        try {
            apply(value);
        }
        catch (e) {
            if (isPolicyControlError(e))
    ...
    policyRoute("/policy/kill-switch", "engaged", (v) => runtime.policy.setKillSwitch(v));
    policyRoute("/policy/execution", "enabled", (v) => runtime.policy.setExecutionEnabled(v));
    app.post(`${prefix}/strategies/:id/status`, async (req, reply) => {
        if (!guard(req, reply, CONTROL_SCOPES.policy))
            return;
        if (!writable(req, reply))
            return;
        ...
        if (typeof status !== "string" ||
            !STRATEGY_STATUSES.includes(status))
            return fail(reply, 400, "VALIDATION_ERROR", `status must be one of ${STRATEGY_STATUSES.join(", ")}`);
        ...
        if (!store)
            return fail(reply, 503, "STRATEGIES_UNAVAILABLE", "no strategy runner is mounted in this process: strategies are composed alongside custody, and this daemon was started without a wallet");
        // Unknown id is a 404, never a silent success — an operator pressing
        // "pause" has to be able to tell a no-op from a stale row.
        if (!store.setStatus(req.params.id, status))
    ...
    // ── sign-in page ──────────────────────────────────────────────────────────
    const page = (reply, type, payload) => reply.type(type).header("cache-control", "no-store").send(payload);
    app.get("/login", async (_req, reply) => page(reply, "text/html; charset=utf-8", LOGIN_HTML));
    app.get("/login.css", async (_req, reply) => page(reply, "text/css; charset=utf-8", LOGIN_CSS));
    app.get("/login.js", async (_req, reply) => page(reply, "text/javascript; charset=utf-8", LOGIN_JS));
    ...
    if (hasDashboard) {
        void app.register(fastifyStatic, {
            ...
            setHeaders(reply, filePath) {
                ...
                reply.header("cache-control", name === "index.html"
                    ? "no-store"
    ...
    app.setNotFoundHandler(async (req, reply) => {
        ...
        const reserved = DEFAULT_RESERVED.some((p) => path === p || path.startsWith(`${p}/`));
        const wantsHtml = String(req.headers.accept ?? "").includes("text/html");
        const navigational = req.method === "GET" || req.method === "HEAD";
        if (reserved || !navigational || !wantsHtml)
            return fail(reply, 404, "NOT_FOUND", `no route for ${req.method} ${path}`);
