...
import { aesGcmDecrypt, aesGcmEncrypt, defaultKdfParams, deriveKek, } from "./crypto.js";
...
export class Keystore {
    ...
    static unlock(path, passphrase) {
        if (!existsSync(path))
            throw new Error(`no keystore at ${path}`);
    ...
    put(id, kind, plaintext) {
        const dek = this.#requireDek();
        const buf = typeof plaintext === "string"
            ? Buffer.from(plaintext, "utf8")
            : plaintext;
        const ct = aesGcmEncrypt(dek, buf);
        const last4 = typeof plaintext === "string" && plaintext.length >= 32
            ? plaintext.slice(-4)
            : undefined;
    ...
    use(id, fn) {
        ...
        if (!item)
            throw new Error(`no secret '${id}' in keystore`);
