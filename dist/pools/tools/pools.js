...
import { assertWalletSignableAlone, toTradeIntent, withdrawInputLeg, } from "../intent.js";
import { binSpan, rangeAroundActive, rangeWidthPct, uiPriceOfBin, } from "../meteora/bins.js";
import { decideRebalance } from "../rebalance/decide.js";
import { LIQUIDITY_SHAPES, } from "../types.js";
import { readRugHeat } from "./deps.js";
...
function renderPool(p) {
    const liq = p.liquidityQuote === undefined
        ? "n/a"
        : formatAmount(p.liquidityQuote, p.quoteDecimals, 2);
    const apr = p.feeApr24hPct === undefined ? "n/a" : `${p.feeApr24hPct.toFixed(2)}%`;
    return (`${p.name ?? shortAddr(p.address)}  ${shortAddr(p.address)}\n` +
...
export function makePoolsListTool(deps) {
    async function render(cfg) {
        ...
        if (pools.length === 0) {
            return {
                text: `No ${deps.venue.id} pools found for ${shortAddr(cfg.mint)} in the scanned window. ` +
    ...
    return {
        name: "pools_list",
        category: "lp",
        description: "List concentrated-liquidity pools for a token mint (bin step, active bin, quote-side liquidity, 24h fees, APR). Read-only.",
...
function renderPosition(p) {
    const lower = uiPriceOfBin(p.lowerLevel, p.levelStepBps, p.baseDecimals, p.quoteDecimals);
    const upper = uiPriceOfBin(p.upperLevel, p.levelStepBps, p.baseDecimals, p.quoteDecimals);
    const inRange = p.activeLevel >= p.lowerLevel && p.activeLevel <= p.upperLevel;
    return (`${shortAddr(p.positionAddress)} in ${shortAddr(p.poolAddress)}\n` +
...
export function makePoolsPositionTool(deps) {
    async function load(ctx, cfg) {
        if (cfg.positionAddress && cfg.poolAddress) {
            const one = await deps.venue.getPosition(cfg.poolAddress, cfg.positionAddress, ctx.ownerWallet);
            return one ? [one] : [];
        }
        return deps.venue.listPositions(ctx.ownerWallet, cfg.poolAddress);
    }
    async function render(ctx, cfg) {
    ...
    return {
        name: "pools_position",
        category: "lp",
        description: "Show the wallet’s liquidity positions: bin range, in/out of range, deposited amounts and unclaimed fees. Read-only.",
...
async function inspectMints(ctx, mints) {
    return Promise.all(mints.map((m) => ctx.services.solana.getMintInfo(m).catch(() => null)));
}
export function makePoolsOpenTool(deps) {
    async function build(ctx, cfg) {
        ...
        const baseAmount = cfg.baseAmountUi
            ...
            : 0n;
        const range = rangeAroundActive(pool.activeLevel, cfg.belowBins ?? 10, cfg.aboveBins ?? 10, deps.guards.maxLevelSpan);
        const shape = cfg.shape ?? "spot";
        const [mints, openPositions, baseHoldings, existing] = await Promise.all([
            ...
            baseAmount > 0n
                ...
                : Promise.resolve(0n),
            cfg.positionAddress
                ? deps.venue.getPosition(cfg.poolAddress, cfg.positionAddress, ctx.ownerWallet)
        ...
        const widthPct = rangeWidthPct(range.lowerBinId, range.upperBinId, pool.levelStepBps);
        const summary = `${cfg.positionAddress ? "Add" : "Open"} ${shape} liquidity in ${pool.name ?? shortAddr(pool.address)}: ` +
        ...
        const warnings = [];
        if (baseAmount > 0n) {
            warnings.push("two-sided deposit: the kernel caps only the quote leg; the base leg is bounded by the pools guard");
        }
    ...
    return {
        name: "pools_open",
        category: "lp",
        description: "Open or add to a concentrated-liquidity position around the active bin. Builds an intent for the kernel; never signs.",
...
export function makePoolsCloseTool(deps) {
    async function build(ctx, cfg) {
        const bps = cfg.bpsToRemove ?? 10_000;
        const pool = await deps.venue.getPool(cfg.poolAddress);
        const position = await deps.venue.getPosition(cfg.poolAddress, cfg.positionAddress, ctx.ownerWallet);
        if (!position)
            throw new PoolGuardError("POOL_VENUE_ERROR", `position ${cfg.positionAddress} not found in ${cfg.poolAddress}`);
        if (position.owner !== ctx.ownerWallet) {
            throw new PoolGuardError("POOL_VENUE_ERROR", "that position is not owned by this wallet");
        }
        ...
        const summary = `Withdraw ${(bps / 100).toFixed(2)}% of ${shortAddr(cfg.positionAddress)} in ${pool.name ?? shortAddr(pool.address)}` +
            `${cfg.claimFees === false ? "" : " and claim fees"}` +
            ` — returns ~${formatAmount((position.baseAmount * BigInt(bps)) / 10000n, position.baseDecimals, 4)} base + ` +
            `${formatAmount((position.quoteAmount * BigInt(bps)) / 10000n, position.quoteDecimals, 4)} quote`;
    ...
    return {
        name: "pools_close",
        category: "lp",
        description: "Withdraw liquidity from a position (optionally all of it, claiming fees and closing the account). Builds an intent; never signs.",
        capabilities: ["sign", "spend", "network"],
        execPolicy: { timeoutMs: 40_000, retries: 0, idempotent: false },
...
export function makePoolsRebalanceTool(deps) {
    async function decide(ctx, cfg) {
        const [pool, position] = await Promise.all([
            deps.venue.getPool(cfg.poolAddress),
            deps.venue.getPosition(cfg.poolAddress, cfg.positionAddress, ctx.ownerWallet),
        ]);
        if (!position)
            throw new PoolGuardError("POOL_VENUE_ERROR", `position ${cfg.positionAddress} not found`);
        if (position.owner !== ctx.ownerWallet) {
            throw new PoolGuardError("POOL_VENUE_ERROR", "that position is not owned by this wallet");
        }
        ...
        const notionalQuote = position.quoteAmount + baseValuedInQuote(position, pool.activePrice);
        // Fee projection: the pool's own 24h fee/TVL ratio applied to our notional.
        const aprPct = pool.feeApr24hPct;
        const projectedFeesPerDayQuote = aprPct !== undefined && Number.isFinite(aprPct) && aprPct >= 0
        ...
        const decision = decideRebalance({
            ...
            economics: {
                ...
                txCostQuote: quoteCostOfLamports(2n * BigInt(priorityFeeLamports + 10_000), pool),
                // Re-centring crosses inventory through the pool at its own fee.
                inventorySwapCostQuote: (notionalQuote * BigInt(Math.max(1, pool.baseFeeBps))) / 10000n,
    ...
    async function build(ctx, cfg) {
        ...
        if (decision.economics) {
            ...
            lines.push(`Economics (quote base units): projected fees ${e.projectedFeesQuote} · cash cost ${e.cashCostQuote} · ` +
        ...
        const summary = `Rebalance step 1/2 — exit ${shortAddr(cfg.positionAddress)} (bins ${position.lowerLevel}..${position.upperLevel}), ` +
        ...
        const followUp = {
            ...
            config: {
                poolAddress: cfg.poolAddress,
                quoteAmountUi: Number(position.quoteAmount) / 10 ** position.quoteDecimals,
                belowBins: pool.activeLevel - target.lowerBinId,
                aboveBins: target.upperBinId - pool.activeLevel,
                shape: deps.rebalancePolicy.shape,
            },
        };
        lines.push("", summary, "", "Step 2/2 (run after step 1 confirms):", JSON.stringify(followUp.config));
        return {
            text: lines.join("\n"),
            built: {
                intent,
    ...
    return {
        ...
        category: "lp",
        description: "Decide whether a liquidity position has drifted enough to re-centre (drift, interval, daily cap and fee-vs-cost economics), and build the exit intent when it has.",
        capabilities: ["sign", "spend", "network"],
        execPolicy: { timeoutMs: 40_000, retries: 0, idempotent: false },
 ...
 */
function baseValuedInQuote(position, uiPrice) {
    if (position.baseAmount <= 0n)
        return 0n;
    if (!Number.isFinite(uiPrice) || uiPrice <= 0)
        return 0n;
    const value = (Number(position.baseAmount) / 10 ** position.baseDecimals) *
        uiPrice *
...
function quoteCostOfLamports(lamports, pool) {
    if (pool.quoteDecimals === 9)
        return lamports;
