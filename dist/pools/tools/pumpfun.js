...
import { z } from "zod";
import { SOL_DECIMALS, WSOL_MINT, formatAmount, fromBaseUnits, toBaseUnits, } from "../../kernel/money.js";
import { PoolGuardError, throwRefusal } from "../errors.js";
import { guardCurveBuy, guardCurveSell } from "../guards.js";
import { toTradeIntent } from "../intent.js";
import { buildBuyInstruction, buildSellInstruction, buildUnsignedTx, ensureUserAtaInstruction, pickFeeRecipient, } from "../pumpfun/instructions.js";
import { quoteBuyForSolBudget, quoteSell } from "../pumpfun/math.js";
import { routeForCurve, routeForMissingCurve, } from "../pumpfun/migration.js";
import { readRugHeat } from "./deps.js";
import { shortAddr } from "./util.js";
function delegation(reason, cfg) {
    return { delegateTo: "swap_jupiter", reason, config: cfg };
}
async function loadCurve(deps, mint) {
    const curve = await deps.curve.readCurve(mint);
    if (!curve)
        return { curve: null, route: routeForMissingCurve(mint) };
    return { curve, route: routeForCurve(curve) };
}
function feeRecipientFor(curve, seed) {
    return pickFeeRecipient(curve.global.feeRecipients, curve.global.feeRecipient, seed);
}
function ixContext(curve, user, seed) {
    if (!curve.creator) {
        throw new PoolGuardError("POOL_UNSUPPORTED_CURVE", `${curve.mint}: curve has no creator field; creator_vault cannot be derived`);
    }
    return {
...
function deriveProgramId(_curve) {
...
export function makePumpfunCurveTool(deps) {
    async function render(cfg) {
        ...
        if (heat)
            ...
            lines.push("rug-heat unavailable (no signals engine / cold tape) — buys will be refused until the tape warms");
        if (route.refusal)
    ...
    return {
        name: "pumpfun_curve",
        category: "launchpad",
        description: "Inspect a pump.fun bonding curve: price, reserves, fee tier, progress to migration, and whether to trade the curve or route via Jupiter. Read-only.",
...
export function makePumpfunBuyTool(deps) {
    async function build(ctx, cfg) {
        ...
        const quote = quoteBuyForSolBudget(curve, solBudget, curve.feeBps, slippageBps);
        const { blockhash, lastValidBlockHeight } = await deps.curve.getLatestBlockhash();
        const ctxIx = ixContext(curve, ctx.ownerWallet, Number(curve.virtualSolReserves % 7n));
        ...
        const summary = `Buy ${tokensUi.toLocaleString("en-US", { maximumFractionDigits: 2 })} ${shortAddr(cfg.mint)} on the pump.fun curve ` +
        ...
        if (curve.progressPct > 90)
            warnings.push(`curve is ${curve.progressPct.toFixed(1)}% complete — it may migrate mid-flight`);
        if (curve.feeSource === "fallback")
            warnings.push("fee tier could not be read on-chain; a fallback rate was used");
    ...
    return {
        name: "pumpfun_buy",
        category: "launchpad",
        description: "Buy a pre-migration pump.fun token directly on its bonding curve, sized in SOL. Migrated tokens are delegated to swap_jupiter. Builds an intent; never signs.",
...
export function makePumpfunSellTool(deps) {
    async function build(ctx, cfg) {
        const { curve, route } = await loadCurve(deps, cfg.mint);
        ...
        if (!curve || route.route === "jupiter") {
            ...
            const amountUi = cfg.amountUi ??
                fromBaseUnits((holdings * BigInt(Math.round(cfg.percent ?? 100))) / 100n, decimals);
            return delegation(route.reason, {
                inputMint: cfg.mint,
        ...
        if (route.refusal)
            throwRefusal(route.refusal);
        const slippageBps = cfg.slippageBps ?? DEFAULT_CURVE_SLIPPAGE_BPS;
        const tokenAmount = cfg.amountUi !== undefined
            ? toBaseUnits(cfg.amountUi, curve.tokenDecimals)
            : (holdings * BigInt(Math.round(cfg.percent ?? 100))) / 100n;
        if (tokenAmount <= 0n)
            throw new PoolGuardError("POOL_SPEND_CAP", "nothing to sell — wallet holds none of this token");
        if (tokenAmount > holdings) {
            throw new PoolGuardError("POOL_SPEND_CAP", `wallet holds ${holdings} base units, cannot sell ${tokenAmount}`);
        }
        ...
        const summary = `Sell ${formatAmount(tokenAmount, curve.tokenDecimals, 2)} ${shortAddr(cfg.mint)} on the pump.fun curve ` +
        ...
        return {
            ...
            meta: {
                ...
                warnings: curve.feeSource === "fallback"
    ...
    return {
        name: "pumpfun_sell",
        category: "launchpad",
        description: "Sell a pre-migration pump.fun token directly on its bonding curve, by amount or percentage of holdings. Migrated tokens are delegated to swap_jupiter. Builds an intent; never signs.",
        capabilities: ["sign", "spend", "network"],
        execPolicy: { timeoutMs: 30_000, retries: 0, idempotent: false },
