 ...
 */
import { SOL_DECIMALS, WSOL_MINT } from "../kernel/money.js";
...
export function withdrawInputLeg(args) {
    ...
    const amount = BigInt(Math.max(0, Math.floor(args.priorityFeeLamports))) +
 ...
 */
export function toTradeIntent(d) {
    if (d.input.amount <= 0n) {
        throw new PoolGuardError("POOL_SPEND_CAP", `${d.source}: input leg must be positive`);
    }
    if (d.minOut > d.expectedOut) {
        throw new PoolGuardError("POOL_SLIPPAGE", `${d.source}: minOut ${d.minOut} exceeds expectedOut ${d.expectedOut}`);
    }
    if (!d.unsignedTxBase64) {
        throw new PoolGuardError("POOL_VENUE_ERROR", `${d.source}: no transaction was built`);
    }
    if (!Number.isFinite(d.priorityFeeLamports) || d.priorityFeeLamports < 0) {
        throw new PoolGuardError("POOL_VENUE_ERROR", `${d.source}: priority fee must be finite and non-negative`);
    }
    const quote = {
 ...
 */
export function assertWalletSignableAlone(draft, source) {
    if (draft.extraSigners.length > 0) {
        throw new PoolGuardError("POOL_EXTRA_SIGNER", `${source}: this transaction needs ${draft.extraSigners.length} signer(s) beyond the wallet ` +
            `(${draft.extraSigners.join(", ")}); the kernel signs one key and will not be handed a second`, { extraSigners: [...draft.extraSigners] });
    }
}
