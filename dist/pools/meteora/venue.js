...
import { compileV0, } from "./sdk-port.js";
 ...
 */
export class MeteoraDlmmVenue {
    id = METEORA_DLMM_VENUE_ID;
    #sdk;
    #api;
    #chain;
    constructor(deps) {
    ...
    async listPools(query) {
        if (!this.#api) {
            throw new PoolGuardError("POOL_VENUE_ERROR", "pool discovery needs the Meteora data API; none was configured");
        }
        const { pools } = await this.#api.listPoolsForMint(query.mint, query.limit ?? 10);
        const summaries = pools.map((p) => summaryFromApi(p));
        if (query.minLiquidityQuote === undefined)
            return summaries;
        const floor = query.minLiquidityQuote;
        return summaries.filter((s) => s.liquidityQuote !== undefined && s.liquidityQuote >= floor);
    }
    ...
    async getPosition(poolAddress, positionAddress, owner) {
    ...
    async listPositions(owner, poolAddress) {
        if (poolAddress) {
            const handle = await this.#sdk.openPool(poolAddress);
            const positions = await handle.positionsOf(owner);
    ...
    async buildOpen(req) {
        ...
        return this.#draft(parts.instructions, parts.extraSigners, parts.description, req.owner, req.priorityFeeLamports);
    }
    async buildRemove(req) {
        const handle = await this.#sdk.openPool(req.poolAddress);
        const positions = await handle.positionsOf(req.owner);
        const target = positions.find((p) => p.publicKey === req.positionAddress);
        if (!target)
            throw new PoolGuardError("POOL_VENUE_ERROR", `position ${req.positionAddress} is not owned by ${req.owner}`);
        if (req.bpsToRemove <= 0 || req.bpsToRemove > 10_000) {
            throw new PoolGuardError("POOL_RANGE_INVALID", `bpsToRemove must be 1..10000, got ${req.bpsToRemove}`);
        }
    ...
    async #draft(instructions, extraSigners, description, payer, priorityFeeLamports) {
        const { blockhash, lastValidBlockHeight } = await this.#chain.getLatestBlockhash();
        const { ComputeBudgetProgram } = await import("@solana/web3.js");
        const cuLimit = 600_000; // DLMM deposits touch several bin arrays; be generous.
        const microLamports = Math.floor((Math.max(0, priorityFeeLamports) * 1_000_000) / cuLimit);
        const withBudget = [
...
export function uiToBaseUnits(value, decimals) {
    if (value === undefined ||
...
export function summaryFromSdk(state, api) {
    return {
        ...
        activePrice: uiPriceOfBin(state.activeBinId, state.binStep, state.tokenXDecimals, state.tokenYDecimals),
        ...
        hasToken2022: state.tokenXProgramId === TOKEN_2022 ||
 ...
 */
export function summaryFromApi(p) {
    ...
    if (binStep > 0 && p.current_price !== undefined && p.current_price > 0) {
        try {
            activeLevel = binOfUiPrice(p.current_price, binStep, baseDecimals, quoteDecimals);
            activePrice = uiPriceOfBin(activeLevel, binStep, baseDecimals, quoteDecimals);
        }
        catch {
            activeLevel = 0;
        }
...
export function positionFromSdk(p, state) {
    return {
        venue: METEORA_DLMM_VENUE_ID,
