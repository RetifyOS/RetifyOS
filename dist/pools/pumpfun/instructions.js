...
import { ASSOCIATED_TOKEN_PROGRAM_ID, createAssociatedTokenAccountIdempotentInstruction, TOKEN_PROGRAM_ID, } from "../../chains/solana/spl.js";
import { BUY_DISCRIMINATOR, PUMP_FEE_PROGRAM_ID, PUMP_PROGRAM_ID, SELL_DISCRIMINATOR, } from "./constants.js";
import { associatedBondingCurve, associatedTokenAccount, bondingCurvePda, creatorVaultPda, eventAuthorityPda, feeConfigPda, globalPda, globalVolumeAccumulatorPda, userVolumeAccumulatorPda, } from "./curve.js";
function derive(ctx) {
...
export function buyAccounts(ctx) {
    const d = derive(ctx);
    return [
...
export function sellAccounts(ctx) {
    const d = derive(ctx);
    return [
        { name: "global", pubkey: d.global, isSigner: false, isWritable: false },
        {
...
export function ensureUserAtaInstruction(ctx) {
    const tokenProgramId = ctx.tokenProgramId ?? TOKEN_PROGRAM_ID;
    return createAssociatedTokenAccountIdempotentInstruction(ctx.user, associatedTokenAccount(ctx.mint, ctx.user, tokenProgramId), ctx.user, ctx.mint, tokenProgramId, ASSOCIATED_TOKEN_PROGRAM_ID);
}
...
export function buildUnsignedTx(args) {
    ...
    if (!Number.isFinite(args.priorityFeeLamports) ||
    ...
    const microLamports = Math.floor((args.priorityFeeLamports * 1_000_000) / cuLimit);
...
export function pickFeeRecipient(recipients, primary, seed) {
    if (recipients.length === 0)
        return new PublicKey(primary);
    const idx = Math.abs(Math.floor(seed)) % recipients.length;
    return new PublicKey(recipients[idx]);
}
