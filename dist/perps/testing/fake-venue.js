 ...
 */
import { USDC_DECIMALS, USDC_MINT, } from "../../kernel/money.js";
 ...
 */
export const FAKE_BASE_PRECISION = 1000000000n;
export function fakeMarket(overrides = {}) {
    return {
        venue: "fake",
        symbol: "SOL-PERP",
        venueMarketIndex: 0,
...
export function fakePrices(overrides = {}) {
    return {
        symbol: "SOL-PERP",
        markPrice: 150,
        oraclePrice: 150,
...
export function fakeFunding(overrides = {}) {
    return {
        symbol: "SOL-PERP",
        bpsPerHour: 0.1,
...
export function fakePosition(overrides = {}) {
    return {
        venue: "fake",
        symbol: "SOL-PERP",
        side: "long",
        ...
        entryPrice: 150,
        markPrice: 150,
        notional: usdc(150000000n), // 150 USDC
        collateral: usdc(50000000n), // 50 USDC
        unrealizedPnl: 0n,
...
export class FakePerpsVenue {
    ...
    #guardRead() {
        if (this.readError)
            throw new PerpsVenueError(this.id, this.readError);
    }
    async listMarkets() {
        this.#guardRead();
        return this.markets;
    ...
    async estimateLiquidationPrice(query) {
        ...
        return {
            ...
            distanceBps: price === undefined
                ? undefined
                : Math.floor((Math.abs(this.prices.markPrice - price) /
    ...
    async getAccountStatus(account) {
        ...
        return {
            ...
            freeCollateral: this.accountExists ? usdc(1000000000n) : undefined,
            totalCollateral: this.accountExists ? usdc(1000000000n) : undefined,
        };
    }
    async buildInitializeAccount(_account) {
        if (!this.allowAccountCreation) {
            throw new PerpsVenueError(this.id, "account creation is disabled on this venue instance");
        }
        return this.#build({
    ...
    async buildOpen(req) {
        this.#guardRead();
        const notionalUnits = (req.collateral.amount * BigInt(Math.round(req.leverage * 10_000))) /
            10000n;
    ...
    async buildAdjust(req) {
        this.#guardRead();
        const priceUnits = BigInt(Math.round(this.prices.markPrice * 1_000_000));
        const notionalUnits = (req.baseAmountDelta * priceUnits) / FAKE_BASE_PRECISION;
        return this.#build({
            market: req.market,
    ...
    async buildClose(req) {
        ...
        if (!pos)
            throw new PerpsVenueError(this.id, `no open ${req.market.symbol} position`);
        const baseAmount = (pos.baseAmount * BigInt(req.fractionBps)) / 10000n;
        const priceUnits = BigInt(Math.round(this.prices.markPrice * 1_000_000));
        return this.#build({
            market: req.market,
            baseAmount,
    ...
    #build(args) {
        const build = {
            ...
            minBaseAmount: (args.baseAmount * BigInt(10_000 - args.slippageBps)) / 10000n,
