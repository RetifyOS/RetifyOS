 ...
 */
import { formatAmount, quoteBucketFor, } from "../kernel/money.js";
import { isPerpGuardError, PerpGuardError, } from "./errors.js";
import { assertPerpIntentShape, isOpeningKind, isReducingKind, } from "./intent.js";
import { liquidationDistanceBps, modelLiquidationPrice, oracleDivergenceBps, reconcileLiquidation, } from "./liquidation.js";
import { capsFor, notionalFromCollateral } from "./policy.js";
function v(code, message, details) {
 ...
 */
const armState = (i, ctx) => {
    if (ctx.killSwitch && isOpeningKind(i.kind)) {
        return v("KILL_SWITCH", "kill switch is engaged — perp exposure may only be reduced");
    }
    if (!ctx.dryRun && !ctx.executionEnabled) {
        return v("PERPS_DISABLED", "execution is disabled — Aetheria is in dry-run. Use /enable to arm.");
    }
    if (!ctx.policy.perpsEnabled && isOpeningKind(i.kind)) {
        return v("PERPS_DISABLED", "perps are not enabled — only reducing an existing position is permitted");
    }
    if (ctx.policy.windDownOnly && isOpeningKind(i.kind)) {
        return v("WIND_DOWN_ONLY", "wind-down mode is active — reduce-only intents may proceed, opens may not");
    }
    return null;
};
const accountReady = (i, ctx) => {
    if (ctx.accountInitialized)
        return null;
    return v("ACCOUNT_NOT_INITIALIZED", `no ${i.perp.venue} subaccount ${i.perp.accountSubId} — initialise it as a separate, explicit step`, {
        venue: i.perp.venue,
        subAccountId: i.perp.accountSubId,
    });
};
// ── market admissibility ─────────────────────────────────────────────────────
const marketAllowed = (i, ctx) => {
    ...
    if (allow && !allow.some((m) => m.toUpperCase() === market)) {
        return v("MARKET_DENIED", `market ${i.perp.market} is not on the allowlist`);
    }
    return null;
};
const marketTradeable = (i) => {
    const status = i.perp.marketStatus;
    if (status === "active")
        return null;
    // A reduce-only market still permits getting smaller.
    if (status === "reduce-only" && isReducingKind(i.kind))
        return null;
    return v("MARKET_NOT_TRADEABLE", `market ${i.perp.market} status is '${status}'`, { status });
};
// ── collateral admissibility (the input leg) ─────────────────────────────────
const collateralAllowed = (i, ctx) => {
    const mint = i.perp.collateral.mint;
    if (ctx.policy.allowedCollateralMints.length === 0) {
        return v("COLLATERAL_MINT_DENIED", "no collateral mint is permitted for perps");
    }
    if (!ctx.policy.allowedCollateralMints.includes(mint)) {
        return v("COLLATERAL_MINT_DENIED", `collateral mint ${mint} is not permitted as perp margin`, { mint });
    }
    return null;
};
/**
 ...
 */
const collateralCappable = (i) => {
    if (bucketOf(i) !== null)
        return null;
    return v("COLLATERAL_NOT_CAPPABLE", `collateral ${i.perp.collateral.mint} has no spend-cap bucket — cannot be bounded without a price oracle`, {
...
// ── order shape ──────────────────────────────────────────────────────────────
const slippageWithinClamp = (i, ctx) => {
    if (i.perp.slippageBps <= ctx.policy.maxSlippageBps)
        return null;
    return v("SLIPPAGE_EXCEEDED", `slippage ${i.perp.slippageBps}bps exceeds max ${ctx.policy.maxSlippageBps}bps`, {
 ...
 */
const reduceIsAReduction = (i, ctx) => {
    if (!isReducingKind(i.kind))
        return null;
    const pos = ctx.position;
    if (!pos)
        return v("NO_POSITION", `no open ${i.perp.market} position to ${i.kind === "perp_close" ? "close" : "reduce"}`);
    if (pos.symbol.toUpperCase() !== i.perp.market.toUpperCase()) {
        return v("POSITION_SIDE_MISMATCH", "position context is for a different market");
    }
    if (pos.side === i.perp.side) {
        return v("POSITION_SIDE_MISMATCH", `a ${i.perp.side} order does not reduce a ${pos.side} position — it increases it`, {
            positionSide: pos.side,
            orderSide: i.perp.side,
        });
    }
    if (i.perp.expectedBaseAmount > pos.baseAmount) {
        return v("SIZE_EXCEEDS_POSITION", "order size exceeds the position it targets — this would flip the side, not reduce it", {
...
const increaseMatchesPosition = (i, ctx) => {
    if (i.kind !== "perp_increase")
        return null;
    const pos = ctx.position;
    if (!pos)
        return v("NO_POSITION", `no open ${i.perp.market} position to increase`);
    if (pos.side !== i.perp.side) {
        return v("POSITION_SIDE_MISMATCH", `cannot increase a ${pos.side} position with a ${i.perp.side} order`, {
            positionSide: pos.side,
            orderSide: i.perp.side,
        });
    }
...
const leverageWithinCap = (i, ctx) => {
    if (!isOpeningKind(i.kind))
        return null;
    const lev = i.perp.leverage;
    if (lev > ctx.policy.maxLeverage) {
        return v("LEVERAGE_EXCEEDED", `leverage ${lev}× exceeds the ${ctx.policy.maxLeverage}× cap`, { leverage: lev, max: ctx.policy.maxLeverage });
    }
    return null;
};
const collateralWithinCap = (i, ctx) => {
    const bucket = bucketOf(i);
    if (bucket === null)
        return null; // collateralCappable already refused this
    if (i.perp.collateral.amount === 0n)
        return null;
    const cap = capsFor(ctx.policy, bucket).maxCollateralPerPosition;
    if (i.perp.collateral.amount > cap) {
        return v("COLLATERAL_CAP_EXCEEDED", `collateral ${fmt(i.perp.collateral.amount, i.perp.collateral.decimals)} exceeds the per-position cap of ${fmt(cap, i.perp.collateral.decimals)}`, {
            bucket,
 ...
 */
const notionalWithinCap = (i, ctx) => {
    if (!isOpeningKind(i.kind))
        return null;
    const bucket = bucketOf(i);
    if (bucket === null)
        return null;
    const derived = notionalFromCollateral(i.perp.collateral.amount, i.perp.leverage);
    if (derived === undefined) {
        return v("NOTIONAL_CAP_EXCEEDED", "notional could not be derived from the collateral and leverage", {
    ...
    if (effective > cap) {
        return v("NOTIONAL_CAP_EXCEEDED", `notional ${fmt(effective, i.perp.notional.decimals)} exceeds the per-position cap of ${fmt(cap, i.perp.notional.decimals)}`, {
            bucket,
...
const portfolioWithinCap = (i, ctx) => {
    if (!isOpeningKind(i.kind))
        return null;
    const bucket = bucketOf(i);
    if (bucket === null)
    ...
    if (ctx.exposure.stale) {
        return v("EXPOSURE_UNKNOWN", `portfolio perp exposure is unknown (${ctx.exposure.staleReason ?? "stale snapshot"}) — refusing to add to an unbounded total`);
    }
    const derived = notionalFromCollateral(i.perp.collateral.amount, i.perp.leverage) ??
        i.perp.notional.amount;
    const add = i.perp.notional.amount > derived ? i.perp.notional.amount : derived;
    const current = ctx.exposure.notionalByBucket[bucket];
    const cap = capsFor(ctx.policy, bucket).maxPortfolioNotional;
    if (current + add > cap) {
        return v("PORTFOLIO_EXPOSURE_EXCEEDED", `total perp notional ${fmt(current + add, i.perp.notional.decimals)} would exceed the ${fmt(cap, i.perp.notional.decimals)} portfolio cap`, {
            bucket,
...
const positionCountWithinCap = (i, ctx) => {
    if (i.kind !== "perp_open")
        return null; // an increase reuses an existing slot
    if (ctx.exposure.stale) {
        return v("EXPOSURE_UNKNOWN", "portfolio snapshot is stale — cannot count open positions");
    }
    if (ctx.position)
        return null; // already holding this market: not a new slot
    if (ctx.exposure.openPositions >= ctx.policy.maxOpenPositions) {
        return v("POSITION_COUNT_EXCEEDED", `already holding ${ctx.exposure.openPositions} perp positions (max ${ctx.policy.maxOpenPositions})`, {
 ...
 */
const liquidationDistanceOk = (i, ctx) => {
    if (!isOpeningKind(i.kind))
        return null;
    const p = i.perp;
    ...
    if (reconciled.ambiguity !== undefined ||
        reconciled.distanceBps === undefined) {
        return v("LIQUIDATION_UNKNOWN", `liquidation price is unusable: ${reconciled.ambiguity ?? "no distance could be computed"}`, {
            venue: p.venueLiquidationPrice,
            model,
        });
    }
    if (reconciled.distanceBps < ctx.policy.minLiquidationDistanceBps) {
        return v("LIQUIDATION_TOO_CLOSE", `entry sits ${reconciled.distanceBps}bps from liquidation, inside the ${ctx.policy.minLiquidationDistanceBps}bps floor`, {
...
const oracleAgreesWithMark = (i, ctx) => {
    if (!isOpeningKind(i.kind))
        return null;
    ...
    if (div > ctx.policy.maxOracleDivergenceBps) {
        return v("ORACLE_DIVERGENCE", `mark and oracle diverge by ${div}bps (max ${ctx.policy.maxOracleDivergenceBps}bps)`, {
            divergenceBps: div,
 ...
 */
const fundingSane = (i, ctx) => {
    if (!isOpeningKind(i.kind))
        return null;
    const f = i.perp.fundingBpsPerHour;
    if (f === undefined || !Number.isFinite(f)) {
        return v("FUNDING_RATE_UNKNOWN", "no funding-rate reading for this market — refusing to open blind");
    }
    if (Math.abs(f) > ctx.policy.maxFundingRateBpsPerHour) {
        return v("FUNDING_RATE_UNSANE", `funding ${f.toFixed(3)}bps/h is beyond the ${ctx.policy.maxFundingRateBpsPerHour}bps/h sanity bound`, {
            fundingBpsPerHour: f,
    ...
    if (adverse > ctx.policy.maxAdverseFundingBpsPerHour) {
        return v("FUNDING_RATE_ADVERSE", `a ${i.perp.side} here pays ${adverse.toFixed(3)}bps/h funding, above the ${ctx.policy.maxAdverseFundingBpsPerHour}bps/h budget`, {
 ...
 */
export function perpGuards(intent, ctx) {
    assertPerpIntentShape(intent);
    for (const rule of PERP_RULES) {
        const violation = rule(intent, ctx);
        if (violation)
            throw new PerpGuardError(violation.code, violation.message, violation.details);
    }
}
/**
 ...
 */
export function evaluatePerpGuards(intent, ctx) {
    try {
        assertPerpIntentShape(intent);
    }
    catch (err) {
        ...
        return {
            ok: false,
            violations: [
                v("INVALID_PERP_INTENT", err instanceof Error ? err.message : String(err), undefined),
            ],
        };
    }
    const violations = [];
 ...
 */
export function positionLiquidationDistanceBps(position) {
