...
async function render(deps, cfg) {
    ...
    if (shown.length === 0)
        lines.push("(no markets matched)");
    lines.push("", `policy: max ${policy.maxLeverage}× · min liq distance ${policy.minLiquidationDistanceBps}bps · perps ${policy.perpsEnabled ? "enabled" : "DISABLED"}`);
    return lines.join("\n");
}
/** List the venue's perp markets with their margin parameters. Read-only — never builds an intent. */
export function makePerpsMarketsTool(deps) {
    return {
        name: "perps_markets",
        category: "perps",
        description: "List available perp markets with max leverage, maintenance margin, taker fee, status and (optionally) mark price + funding.",
