...
import { accountRef, errMsg } from "./deps.js";
const configSchema = z.object({ market: z.string().optional() });
async function render(deps, ctx, cfg) {
    ...
    if (status.totalCollateral) {
        lines.push(`collateral ${formatAmount(status.totalCollateral.amount, status.totalCollateral.decimals, 2)}` +
    ...
    for (const p of positions) {
        ...
        const liq = p.liquidationPrice === undefined
            ? "liq n/a"
            : `liq $${p.liquidationPrice.toLocaleString("en-US", { maximumFractionDigits: 4 })}`;
        const distTxt = dist === undefined ? "" : ` (${dist}bps away)`;
        lines.push(`${p.symbol} ${p.side} ${size} · notional ${notional} · entry $${p.entryPrice.toFixed(4)} · mark $${p.markPrice.toFixed(4)} · pnl ${pnl} · ${liq}${distTxt}`);
    }
    for (const bucket of ["sol", "usdc"]) {
        ...
        lines.push(`${bucket.toUpperCase()} perp notional ${formatAmount(used, decimals, 2)} / ${formatAmount(cap, decimals, 2)} cap`);
    }
    if (exposure.stale)
        lines.push(`! exposure snapshot is incomplete: ${exposure.staleReason ?? "unknown"} — opens will be refused`);
    return lines.join("\n");
}
/** List open perp positions with liquidation distance and portfolio-cap usage. Read-only. */
export function makePerpsPositionsTool(deps) {
    return {
        name: "perps_positions",
        category: "perps",
        description: "List open perp positions with size, notional, entry/mark, unrealized PnL, liquidation distance and portfolio-cap usage.",
