...
async function build(deps, ctx, cfg, dryRun) {
    ...
    if (!position) {
        throw new PerpGuardError("NO_POSITION", `no open ${market.symbol} position to close`, { market: market.symbol });
    }
 ...
 */
export function makePerpsCloseTool(deps) {
    return {
        name: "perps_close",
        category: "perps",
        description: "Propose closing all or part of an open perp position (reduce-only). Builds an intent for the kernel; never executes.",
