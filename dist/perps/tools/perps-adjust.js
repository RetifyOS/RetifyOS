...
import { accountRef, DEFAULT_SLIPPAGE_BPS, priorityFee, readSnapshot, } from "./deps.js";
import { makeProposal, proposalPreview, proposalResult, readFunding, } from "./propose.js";
...
async function build(deps, ctx, cfg, dryRun) {
    ...
    if (!position) {
        throw new PerpGuardError("NO_POSITION", `no open ${market.symbol} position to adjust`, { market: market.symbol });
    }
 ...
 */
export function makePerpsAdjustTool(deps) {
    return {
        name: "perps_adjust",
        category: "perps",
        description: "Propose increasing or reducing an open perp position by a base-size delta. Builds an intent for the kernel; never executes.",
