...
import { absBigInt, BASE_DECIMALS, BASE_PRECISION, bnToBigInt, bnToNumber, decodeMarketName, MARGIN_PRECISION, PRICE_PRECISION, QUOTE_DECIMALS, } from "./sdk-types.js";
/**
 ...
 */
export function driftMarketStatus(key) {
    switch (key) {
        case "active":
            return "active";
        case "reduceOnly":
 ...
 */
export function fundingBpsPerHour(lastFundingRate, oracleTwap) {
...
export function marginRatioToFraction(ratio) {
...
export function maxLeverageFromInitialMargin(initialMarginFraction) {
 ...
 */
export function toPerpMarket(account, takerFeeBps) {
    ...
    const step = bnToBigInt(account.amm.orderStepSize ?? account.amm.baseAssetAmountStepSize) ?? 0n;
...
function anchorStatusKey(account) {
 ...
 */
export function notionalQuoteUnits(baseUnits, pricePrecisionPrice) {
...
export function priceFrom(value, opts = {}) {
    const raw = bnToBigInt(value);
    if (raw === undefined)
        return undefined;
    if (raw < 0n)
        return opts.negativeMeans === "zero" ? 0 : undefined;
