...
import { DRIFT_SETTLEMENT_MINT, DRIFT_VENUE_ID, fundingBpsPerHour, notionalQuoteUnits, priceFrom, splitSignedBase, toPerpMarket, } from "./convert.js";
import { BASE_PRECISION, bnToBigInt, PRICE_PRECISION, QUOTE_PRECISION, } from "./sdk-types.js";
 ...
 */
export function readOnlyWallet(publicKey) {
    const refuse = () => {
        throw new PerpsVenueError(DRIFT_VENUE_ID, "the Drift adapter is signing-incapable by construction — only the kernel signs");
    };
    return {
        publicKey,
        payer: undefined,
...
export class DriftVenue {
    ...
    async #doConnect() {
        ...
        if (!ok)
            throw new PerpsVenueError(this.id, "DriftClient.subscribe() returned false");
        this.#sdk = sdk;
        this.#client = client;
    }
    async disconnect() {
    ...
    async #loadSdk() {
        if (this.#opts.sdkLoader)
            return this.#opts.sdkLoader();
        ...
        try {
            ...
            throw new PerpsVenueError(this.id, '@drift-labs/sdk is not installed — run "npm install @drift-labs/sdk" to run perps ' +
    ...
    #ready() {
        if (!this.#client || !this.#sdk) {
            throw new PerpsVenueError(this.id, "DriftVenue.connect() must be awaited before use");
        }
    ...
    async listMarkets() {
        ...
        const accounts = this.#call(() => client.getPerpMarketAccounts(), "getPerpMarketAccounts");
        const out = [];
    ...
    async getMarket(symbol) {
        ...
        if (!found)
            throw new PerpsVenueError(this.id, `unknown or unusable perp market '${symbol}'`);
        return found;
    }
    #marketAccount(marketIndex) {
        const { client } = this.#ready();
        const account = this.#call(() => client.getPerpMarketAccount(marketIndex), "getPerpMarketAccount");
        if (!account)
            throw new PerpsVenueError(this.id, `no Drift perp market account at index ${marketIndex}`);
        return account;
    }
    async getPrices(symbol) {
        ...
        const oracle = this.#call(() => client.getOracleDataForPerpMarket(market.venueMarketIndex), "getOracleDataForPerpMarket");
        const oraclePrice = priceFrom(oracle.price);
        if (oraclePrice === undefined || oraclePrice <= 0) {
            throw new PerpsVenueError(this.id, `${market.symbol}: no usable oracle price`);
        }
    ...
    async getFundingRate(symbol) {
        const market = await this.getMarket(symbol);
        const account = this.#marketAccount(market.venueMarketIndex);
        const twap = bnToBigInt(account.amm.historicalOracleData?.lastOraclePriceTwap);
        ...
        if (bps === undefined) {
            ...
            throw new PerpsVenueError(this.id, `${market.symbol}: no usable funding rate`);
        }
        return {
            symbol: market.symbol,
            bpsPerHour: bps,
    ...
    async getPositions(account) {
        const { client } = this.#ready();
        const user = this.#call(() => client.getUser(account.subAccountId), "getUser");
        const userAccount = this.#call(() => user.getUserAccount(), "getUserAccount");
        const markets = await this.listMarkets();
        const out = [];
        for (const raw of userAccount.perpPositions ?? []) {
            const signed = bnToBigInt(raw.baseAssetAmount);
            if (signed === undefined || signed === 0n)
                continue;
            const market = markets.find((m) => m.venueMarketIndex === raw.marketIndex);
            if (!market)
                continue;
            const oracle = this.#call(() => client.getOracleDataForPerpMarket(raw.marketIndex), "getOracleDataForPerpMarket");
            ...
            const entryPrice = magnitude > 0n
                ? Math.abs(Number(quoteEntry) / Number(QUOTE_PRECISION)) /
                    (Number(magnitude) / Number(BASE_PRECISION))
                : 0;
            const liq = this.#safe(() => priceFrom(user.liquidationPrice(raw.marketIndex)));
            const collateralUnits = this.#safe(() => bnToBigInt(user.getTotalCollateral())) ?? 0n;
            const leverage = collateralUnits > 0n
            ...
            const unrealizedPnl = (bnToBigInt(raw.quoteAssetAmount) ?? 0n) +
    ...
    async estimateLiquidationPrice(query) {
        const { client, sdk } = this.#ready();
        ...
        const venuePrice = this.#safe(() => {
            ...
            const delta = new sdk.BN((query.side === "long"
                ? query.baseAmount
                : -query.baseAmount).toString());
            const entry = new sdk.BN(Math.round(query.entryPrice * Number(PRICE_PRECISION)).toString());
            return priceFrom(user.liquidationPrice(query.market.venueMarketIndex, delta, entry));
        });
        const collateralUi = Number(query.collateral.amount) / 10 ** query.collateral.decimals;
        const notionalUi = (Number(query.baseAmount) / Number(BASE_PRECISION)) * query.entryPrice;
        ...
        const distanceBps = price === undefined
            ? undefined
            : Math.floor((Math.abs(prices.markPrice - price) / prices.markPrice) * 10_000);
        return {
    ...
    async getAccountStatus(account) {
        const { client } = this.#ready();
        const userAccount = this.#safe(() => client.getUserAccount(account.subAccountId));
        const exists = Boolean(userAccount);
        const free = exists
            ? this.#safe(() => bnToBigInt(client.getUser(account.subAccountId).getFreeCollateral()))
            : undefined;
        const total = exists
            ? this.#safe(() => bnToBigInt(client.getUser(account.subAccountId).getTotalCollateral()))
            : undefined;
        return {
            venue: this.id,
            owner: account.owner,
     ...
     */
    async buildInitializeAccount(account) {
        if (this.#opts.allowAccountCreation !== true) {
            throw new PerpGuardError("ACCOUNT_CREATION_DISABLED", "Drift account creation is disabled — enable it explicitly, it costs rent and creates on-chain state", { venue: this.id, subAccountId: account.subAccountId });
        }
        const { client } = this.#ready();
        const status = await this.getAccountStatus(account);
        if (status.exists) {
            throw new PerpsVenueError(this.id, `subaccount ${account.subAccountId} already exists`);
        }
        const ixs = await this.#callAsync(() => client.getInitializeUserInstructions(account.subAccountId), "getInitializeUserInstructions");
        ...
        return this.#assemble([...ixs].filter((ix) => ix !== undefined && ix !== null), {
    ...
    async buildOpen(req) {
        ...
        const notionalUnits = (req.collateral.amount * BigInt(bps)) / 10000n;
        const { entryPrice, priceUnits } = await this.#entry(req.market.symbol, req.side);
        const baseAmount = (notionalUnits * BASE_PRECISION) / priceUnits;
        if (baseAmount <= 0n)
            throw new PerpsVenueError(this.id, "derived base size is zero — collateral or leverage too small");
        return this.#order({
            market: req.market,
            side: req.side,
            baseAmount,
    ...
    async buildAdjust(req) {
        if (req.baseAmountDelta <= 0n)
            throw new PerpsVenueError(this.id, "adjust requires a positive base delta");
        const positions = await this.getPositions(req.account);
        const current = positions.find((p) => p.symbol === req.market.symbol);
        if (!current)
            throw new PerpsVenueError(this.id, `no open ${req.market.symbol} position to adjust`);
        // Increase trades the SAME way as the position; reduce trades the opposite way.
        const side = req.direction === "increase"
            ? current.side
            : current.side === "long"
                ? "short"
                : "long";
        const { entryPrice, priceUnits } = await this.#entry(req.market.symbol, side);
        return this.#order({
            market: req.market,
            side,
    ...
    async buildClose(req) {
        if (req.fractionBps <= 0 || req.fractionBps > 10_000) {
            throw new PerpsVenueError(this.id, `close fraction ${req.fractionBps}bps out of range`);
        }
        const positions = await this.getPositions(req.account);
        const current = positions.find((p) => p.symbol === req.market.symbol);
        if (!current)
            throw new PerpsVenueError(this.id, `no open ${req.market.symbol} position to close`);
        const baseAmount = (current.baseAmount * BigInt(req.fractionBps)) / 10000n;
        if (baseAmount <= 0n)
            throw new PerpsVenueError(this.id, "close size rounds to zero");
        const side = current.side === "long" ? "short" : "long";
        const { entryPrice, priceUnits } = await this.#entry(req.market.symbol, side);
        return this.#order({
            market: req.market,
            side,
    ...
    // ── internals ──
    async #entry(symbol, _side) {
        const prices = await this.getPrices(symbol);
        const priceUnits = BigInt(Math.round(prices.markPrice * Number(PRICE_PRECISION)));
        if (priceUnits <= 0n)
    ...
    async #order(args) {
        ...
        if (args.orderType === "limit" &&
            (args.limitPrice === undefined ||
                ...
                args.limitPrice <= 0)) {
            throw new PerpsVenueError(this.id, "a limit order needs a positive limit price");
        }
        const direction = args.side === "long"
        ...
        if (args.orderType === "limit" && args.limitPrice !== undefined) {
            params.price = new sdk.BN(Math.round(args.limitPrice * Number(PRICE_PRECISION)).toString());
        }
        const build = args.orderType === "limit"
            ? sdk.getLimitOrderParams
            : sdk.getMarketOrderParams;
        if (!build)
            throw new PerpsVenueError(this.id, `SDK does not export a ${args.orderType}-order params builder`);
        const orderParams = build(params);
        const ix = await this.#callAsync(() => client.getPlacePerpOrderIx(orderParams, args.account.subAccountId), "getPlacePerpOrderIx");
        // Worst-case fill size: the same notional bought at a `slippageBps` worse price.
        const minBaseAmount = (args.baseAmount * BigInt(10_000 - Math.min(args.slippageBps, 10_000))) /
            10000n;
        const liq = this.#safe(() => {
            const user = client.getUser(args.account.subAccountId);
            const delta = new sdk.BN((args.side === "long" ? args.baseAmount : -args.baseAmount).toString());
            const entry = new sdk.BN(Math.round(args.entryPrice * Number(PRICE_PRECISION)).toString());
            return priceFrom(user.liquidationPrice(args.market.venueMarketIndex, delta, entry));
        });
     ...
     */
    async #assemble(ixs, extra) {
        const { client } = this.#ready();
        const { blockhash, lastValidBlockHeight } = await this.#opts.blockhash.latestBlockhash();
        const priorityFeeLamports = extra.priorityFeeLamports ??
        ...
        const lookupTables = client.fetchMarketLookupTableAccount
            ...
            : undefined;
        const tx = await this.#callAsync(() => client.buildTransaction(ixs, {
            computeUnitsPrice: priorityFeeLamports,
            recentBlockhash: blockhash,
        }, undefined, lookupTables ? [lookupTables] : undefined), "buildTransaction");
        ...
        if (embedded && embedded !== blockhash) {
            warnings.push("the SDK embedded a different blockhash than the one reported; expiry is terminal either way");
        }
        return {
            unsignedTxBase64,
    ...
    #call(fn, what) {
        try {
            ...
            throw new PerpsVenueError(this.id, `${what} failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    async #callAsync(fn, what) {
        try {
            return await fn();
        }
        catch (err) {
