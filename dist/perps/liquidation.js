...
export function isUsablePrice(x) {
 ...
 */
export function modelLiquidationPrice(args) {
    const { side, entryPrice, leverage, maintenanceMarginRatio: m } = args;
    if (!isUsablePrice(entryPrice))
        return undefined;
...
export function maxLeverageForDistance(minDistanceBps, side, maintenanceMarginRatio) {
    const m = maintenanceMarginRatio;
    if (!Number.isFinite(minDistanceBps) ||
        minDistanceBps < 0 ||
...
export function reconcileLiquidation(args) {
    ...
    const venueDist = venuePrice === undefined
        ? undefined
    ...
    const modelDist = modelPrice === undefined
        ? undefined
        : liquidationDistanceBps({
            side,
            referencePrice,
    ...
    if (venueDist === undefined && modelDist === undefined) {
        return none("neither the venue nor the model produced a usable liquidation price");
    }
    if (venueDist !== undefined && modelDist !== undefined) {
...
export function oracleDivergenceBps(markPrice, oraclePrice) {
