 ...
 */
export function positionReaderFor(venue) {
    return {
        async readPosition(ref) {
            if (ref.venue !== venue.id) {
                throw new PerpsVenueError(venue.id, `cannot read a '${ref.venue}' position from the '${venue.id}' venue`);
            }
 ...
 */
export function positionReaderOver(venues) {
    const byId = new Map(venues.map((v) => [v.id, positionReaderFor(v)]));
    return {
        async readPosition(ref) {
            const reader = byId.get(ref.venue);
            if (!reader) {
                throw new PerpsVenueError(ref.venue, `no perps venue named '${ref.venue}' is mounted — the fill cannot be verified`);
            }
