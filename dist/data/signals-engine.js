...
export class SignalsEngine {
     ...
     */
    rugHeatScore(mint, windowMs = 300_000) {
        ...
        if (imbalance >= 40) {
            score += 25;
            reasons.push(`extreme one-sided flow (${s.volumeWeightedBuyPressurePct.toFixed(0)}% buy volume)`);
        }
        else if (imbalance >= 25) {
            score += 12;
            reasons.push(`lopsided flow (${s.volumeWeightedBuyPressurePct.toFixed(0)}% buy volume)`);
        }
        ...
        if (s.trades >= 4 &&
            ...
            score += 18;
            reasons.push(`buy count high but sell SIZE dominates (${s.buyPressurePct.toFixed(0)}% buys vs ${s.volumeWeightedBuyPressurePct.toFixed(0)}% buy volume)`);
        }
        ...
        if (s.volumeSol > 0 &&
            ...
            score += 12;
            reasons.push(`one trade is ${((s.largestTradeSol / s.volumeSol) * 100).toFixed(0)}% of volume`);
        }
        ...
        if (s.netSolFlow < 0 && s.volumeSol > 0) {
            ...
            if (drainPct >= 60) {
                score += 25;
                reasons.push(`net SOL outflow (${drainPct.toFixed(0)}% of volume selling)`);
            }
            else if (drainPct >= 30) {
                score += 12;
                reasons.push(`net SOL outflow (${drainPct.toFixed(0)}% of volume selling)`);
        ...
        if (reasons.length === 0)
            reasons.push("no strong rug tells in window");
        return { score, reasons };
    }
    /** One-line human summary: windowed signals + rug-heat verdict. */
    summary(mint, windowMs = 300_000) {
        const s = this.#tape.signals(mint, windowMs);
        const heat = this.rugHeatScore(mint, windowMs);
        const mins = Math.round(windowMs / 60_000);
        const price = s.lastPriceSol !== undefined
            ? `${s.lastPriceSol.toPrecision(4)} SOL`
            : "n/a";
        const move = s.priceChangePct !== undefined ? ` (${signed(s.priceChangePct)}%)` : "";
        return (`${short(mint)} · last ${mins}m: ${s.trades} trades ` +
